...
function ProductsToCart() {
  ...
  return (
    <aside className={`${context.isProductsToCartOpen ? 'flex' : 'hidden'} flex-col font-montserrat w-96 h-screen fixed top-0 right-0 bg-white z-10 p-4`}>
    <GrClose className="w-6 h-6 absolute top-5 right-4 cursor-pointer" onClick={() => {context.closeProductsToCart()}}/>
      <h5 className="font-bold text-lg">CARRITO DE COMPRAS</h5>
      <p className="mt-2 mb-8 text-sm font-light text-stone-600">
        1 producto/s agregado/s
      </p>
      ...
      <div className="flex justify-between w-full h-12 items-center border-b border-stone-300">
        ...
        <span className="text-lg">$234.000</span>
      </div>
      ...
      <input type="button" value={'PAGAR'} className="w-full h-12 mt-3 mb-3 font-extrabold text-white text-sm bg-green-600"/>
      <Link to="/">
